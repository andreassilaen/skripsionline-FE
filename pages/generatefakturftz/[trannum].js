...
const DetailTranno = () => {
  ...
  useEffect(() => {
    ...
    if (router.query.trannum !== "undefined") {
      var props = router.query;
      setTrannum(props.trannum);
    } else {
      router.push({
  ...
  useEffect(() => {
    if (trannum !== "") {
      debounceMountGetUnfakturDetail();
    }
  }, [trannum]);

  const debounceMountGetUnfakturDetail = useCallback(
    debounce(mountGetUnfakturDetail, 400)
  );
  ...
  async function mountGetUnfakturDetail() {
    const newParams = {
      ptid: PTID,
      outcode: gudangID,
      trannum: trannum,
    ...
    setIsLoading(true);
    try {
      const getFakturDetail = await api.getUnfakturDetail(newParams);
      ...
      <Modal open={responseModalIsOpen}>
        <Box sx={style}>
          <Grid textAlign="center">
            <Typography fontWeight={600}>{responseHeader}</Typography>
          </Grid>
          <Grid>
