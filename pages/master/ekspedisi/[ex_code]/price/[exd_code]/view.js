...
const ViewEkspedisiPriceDetail = () => {
  const router = useRouter();
  const [displayToast] = useToast();

  const debounceViewEkspedisiPriceDetail = useCallback(
    ...
    <Box sx={{ width: "100%", p: 3 }}>
      <Grid container justifyContent={"space-between"}>
        <Grid container item xs={6}>
          <Link href={`/master/ekspedisi/${params.code}/detail`} sx={{ mr: 2 }}>
        ...
        <Grid container item xs={6} justifyContent={"flex-end"}>
          <Button
            ...
            variant="contained"
            startIcon={<SaveIcon />}
            onClick={() => debounceViewEkspedisiPriceDetail(params, inputValue)}
          >
            Save
          </Button>
        </Grid>
...
export default ViewEkspedisiPriceDetail;
