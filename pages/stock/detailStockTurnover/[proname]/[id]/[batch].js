import {
  ...
  const MutasiStockDetail = (props) => {
    ...
    const debounceMountMutasiStockDetail = useCallback(
      debounce(mountMutasiStockDetail, 400),
      []
    );
    ...
    async function mountMutasiStockDetail(procod, pt, outcode, batch, group, startdate, enddate, params) {
      try {
        setDataAvailable((dataAvailable = true))
        const getMutasiStockDetail = await api.getMutasiStockDetail(procod, pt, outcode, batch, group, startdate, enddate, params);
        const { data, metadata } = getMutasiStockDetail.data;
        // console.log("data mutasi stock detail", getMutasiStockDetail);
    ...
    async function mountHeaderDO(notransf, pt, outcode) {
      try {
        // console.log('hit ke be print', outcode, notransf)
    ...
    const handlePageChange = (event, newPage) => {
      ...
      setDataAvailable(false)
      debounceMountMutasiStockDetail(procod, pt_id, outcode, batch, 0, startDate, endDate, newParams);
    };
    ...
    function getData(){
      if (startDate !== '' && endDate !== '') {
          if(validateTgl() === true){
              ...
              debounceMountMutasiStockDetail(procod, pt_id, outcode, batch, 0, startDate, endDate, params);
              setShowData(true)
          }
      } else {
        ...
        <Grid container justifyContent={"space-between"}>
          <Grid item xs={4} sm={4} md={4} lg={4}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 600, mt: 0.5 }}>
                {language === 'EN' ? 'Detail Inventory Stock Turnover' : 'Mutasi Stock Detail'}
            </Typography>
            </Grid>
            ...
            <Table>
              ...
              (
                  ...
                  <TableRow key={item.stck_batchnumber}>
                    ...
                    <TableCell sx={{ width: "15%" }}>{formatDate(item.mutasi_ed, "ddd MMMM DD YYYY")}</TableCell>
                    ...
                    <TableCell align="center">{item.mutasi_type}</TableCell>
  ...
  export default MutasiStockDetail;
